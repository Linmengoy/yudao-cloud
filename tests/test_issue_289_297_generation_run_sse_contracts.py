...
class GenerationRunSseContractsTest(unittest.TestCase):
    ...
    def test_issue_289_project_sse_requires_read_permission_and_subscribes_by_project(self):
        ...
        endpoint = require_section(
            ...
            r"public SseEmitter subscribeGenerationRunEvents\(@PathVariable\(\"id\"\) Long id\) \{(?P<body>.*?)\n    \}",
        )
        self.assertIn("return nodeRunService.subscribeGenerationRunEvents(id, getLoginUserId());", endpoint)

        subscription = require_section(
            service,
            r"public SseEmitter subscribeGenerationRunEvents\(Long projectId, Long userId\) \{(?P<body>.*?)\n    \}",
        )
        self.assertIn("projectService.validateReadableProject(projectId, userId)", subscription)
        self.assertIn("return generationRunSseService.subscribe(projectId);", subscription)
    ...
    def test_issue_289_sse_service_limits_connections_and_forces_initial_resync(self):
        ...
        subscribe = require_section(
            sse,
            r"public SseEmitter subscribe\(Long projectId\) \{(?P<body>.*?)\n    public void publish",
        )
        for required in [
            ...
            self.assertIn(required, subscribe if required != "MAX_PROJECT_CONNECTIONS = 6" else sse)
