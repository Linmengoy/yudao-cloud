...
class ReleaseWorkflowManualDispatchContractsTest(unittest.TestCase):
    ...
    def test_prod_workflow_infers_previous_stable_sha_before_preflight(self):
        ...
        self.assertIn("INPUT_PREVIOUS_STABLE_IMAGE_TAG: ${{ github.event.inputs.previous_stable_image_tag }}", workflow)
        self.assertIn("INPUT_PREVIOUS_STABLE_IMAGE_TAG_SOURCE: ${{ github.event.inputs.previous_stable_image_tag_source }}", workflow)
        self.assertIn('requested_previous_stable_image_tag="${INPUT_PREVIOUS_STABLE_IMAGE_TAG:-}"', workflow)
        self.assertIn('requested_previous_stable_image_tag_source="${INPUT_PREVIOUS_STABLE_IMAGE_TAG_SOURCE:-}"', workflow)
        self.assertIn('previous_stable_image_tag=""', workflow)
        self.assertIn("previous_stable_image_tag_source:", workflow)
        self.assertIn("上一稳定 tag 来源：上一条成功发布 issue、prod-stable-* tag、/opt/deploy/yudao-micro/.env 或 running container", workflow)
        self.assertIn('"previous successful release issue #"*', workflow)
        self.assertIn('*"root/manman/issues/"*', workflow)
        self.assertIn('"prod-stable tag prod-stable-"*', workflow)
        self.assertIn('"/opt/deploy/yudao-micro/.env"', workflow)
        self.assertIn('"running container "*', workflow)
        self.assertIn("previous_stable_image_tag_source must be an allowed auditable source", workflow)
        ...
        self.assertIn('[ -z "${requested_previous_stable_image_tag}" ] || [ "${stable_sha}" = "${requested_previous_stable_image_tag}" ]', workflow)
        ...
        self.assertIn('env_previous_stable_image_tag="$(sed -n \'s/^MICRO_IMAGE_TAG=//p\' "${deploy_env_file}"', workflow)
        self.assertIn('[ -z "${requested_previous_stable_image_tag}" ] || [ "${env_previous_stable_image_tag}" = "${requested_previous_stable_image_tag}" ]', workflow)
        ...
        self.assertIn('running_previous_stable_image_tag="${running_image##*:}"', workflow)
        self.assertIn('[ -z "${requested_previous_stable_image_tag}" ] || [ "${running_previous_stable_image_tag}" = "${requested_previous_stable_image_tag}" ]', workflow)
        self.assertIn('previous_stable_image_tag_source="running container ${container}"', workflow)
        self.assertIn("workflow_dispatch.previous_stable_image_tag must provide or match an allowed previous stable source", workflow)
        self.assertIn("previous_stable_image_tag could not be inferred from prod-stable-* tag", workflow)
        ...
        self.assertIn("previous stable image tag source: ${PREVIOUS_STABLE_IMAGE_TAG_SOURCE}", workflow)
        self.assertNotIn('previous_stable_image_tag_source="workflow_dispatch.previous_stable_image_tag"', workflow)

        self.assertIn("prod 当前 tag 始终使用当前 12 位 Git SHA", runbook)
        self.assertIn("留空时 workflow 会先查 `prod-stable-*` tag", runbook)
        self.assertIn("手填值只作为期望上一稳定 SHA", runbook)
        self.assertIn("必须同时填写 `previous_stable_image_tag_source`", runbook)
        self.assertIn("允许的来源是上一条成功发布 issue、`prod-stable-*` tag、`/opt/deploy/yudao-micro/.env` 或当前运行容器", runbook)
