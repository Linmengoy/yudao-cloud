...
public class GrokImagineProviderClient implements AigcProviderClient {
    ...
    public AigcProviderSubmitRespDTO submit(AigcProviderSubmitReqDTO reqDTO) {
        ...
        try (HttpResponse response = AigcProviderProxyUtils.applyProxy(HttpRequest.post(resolveSubmitEndpoint(reqDTO))
    ...
    public AigcProviderSubmitRespDTO query(AigcProviderSubmitReqDTO reqDTO) {
        ...
        long start = System.currentTimeMillis();
        try (HttpResponse response = AigcProviderProxyUtils.applyProxy(HttpRequest.get(resolveVideoTaskEndpoint(reqDTO, false))
                .header(Header.AUTHORIZATION, "Bearer " + reqDTO.getProviderApiKey())
                .timeout(timeoutMillis(reqDTO)), reqDTO)
                .execute()) {
            if (!response.isOk()) {
    ...
    private String toProviderImage(String image, AigcProviderSubmitReqDTO reqDTO) {
        ...
        try (HttpResponse response = AigcProviderProxyUtils.applyProxy(HttpRequest.get(image)
                .timeout(timeoutMillis(reqDTO)), reqDTO)
                .execute()) {
            if (!response.isOk()) {
    ...
    private String downloadVideoAsDataUrl(AigcProviderSubmitReqDTO reqDTO) {
        try (HttpResponse response = AigcProviderProxyUtils.applyProxy(HttpRequest.get(resolveVideoTaskEndpoint(reqDTO, true))
                .header(Header.AUTHORIZATION, "Bearer " + reqDTO.getProviderApiKey())
                .timeout(timeoutMillis(reqDTO)), reqDTO)
                .execute()) {
            if (!response.isOk()) {
