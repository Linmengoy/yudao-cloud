  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="520px">
    <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="100px">
      ...
      <el-form-item label="调整积分" prop="amount">
        <el-input-number v-model="formData.amount" class="!w-240px" :min="-99999999" :max="99999999" />
      </el-form-item>
      <el-form-item label="调整原因" prop="remark">
        <el-input v-model="formData.remark" :rows="3" placeholder="请输入调整原因" type="textarea" />
      </el-form-item>
    </el-form>
    <template #footer>
...
defineOptions({ name: 'AigcBillingWalletAdjustForm' })

const message = useMessage()
const dialogVisible = ref(false)
const dialogTitle = ref('调整余额')
...
const formRules = reactive({
  ...
  remark: [{ required: true, message: '调整原因不能为空', trigger: 'blur' }]
})
...
const submitForm = async () => {
  ...
  try {
    await AigcBillingWalletApi.adjustWallet(formData.value)
