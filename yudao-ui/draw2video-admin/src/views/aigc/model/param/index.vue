  ...
  <ContentWrap>
    <el-form ref="queryFormRef" :model="queryParams" :inline="true" class="-mb-15px" label-width="68px">
      ...
      <el-form-item>
        ...
        <el-button type="primary" plain @click="openForm('create')" v-hasPermi="['aigc:model:param:create']"><Icon icon="ep:plus" class="mr-5px" /> {{ t('aigc.model.actions.create') }}</el-button>
        <el-button type="success" plain @click="openCopyForm" v-hasPermi="['aigc:model:param:create']"><Icon icon="ep:copy-document" class="mr-5px" /> {{ t('aigc.model.actions.copyParams') }}</el-button>
      </el-form-item>
    </el-form>
  </ContentWrap>
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column :label="t('aigc.model.fields.paramKey')" align="center" prop="paramKey" min-width="130" />
      <el-table-column :label="t('aigc.model.fields.paramName')" align="center" prop="paramName" min-width="130" />
      <el-table-column :label="t('aigc.model.fields.type')" align="center" prop="paramType" min-width="90"><template #default="scope">{{ getOptionLabel(AIGC_PARAM_TYPES, scope.row.paramType, t) }}</template></el-table-column>
      <el-table-column :label="t('aigc.model.fields.required')" align="center" prop="requiredStatus" min-width="80"><template #default="scope"><el-tag :type="scope.row.requiredStatus ? 'warning' : 'info'">{{ scope.row.requiredStatus ? t('aigc.model.fields.required') : t('aigc.model.fields.optional') }}</el-tag></template></el-table-column>
      <el-table-column :label="t('aigc.model.fields.defaultValue')" align="center" prop="defaultValue" min-width="120" />
      <el-table-column :label="t('aigc.model.fields.sort')" align="center" prop="sort" min-width="80" />
      ...
      <el-table-column :label="t('table.action')" align="center" width="160" fixed="right"><template #default="scope"><el-button link type="primary" @click="openForm('update', scope.row.id)" v-hasPermi="['aigc:model:param:update']">{{ t('aigc.model.actions.edit') }}</el-button><el-button link type="danger" @click="handleDelete(scope.row.id)" v-hasPermi="['aigc:model:param:delete']">{{ t('aigc.model.actions.delete') }}</el-button></template></el-table-column>
    </el-table>
  </ContentWrap>
