<template>
  <ContentWrap>
    <el-form ref="queryFormRef" :model="queryParams" :inline="true" class="-mb-15px" label-width="72px">
      <el-form-item label="任务 ID" prop="taskId">
        <el-input-number v-model="queryParams.taskId" class="!w-180px" :min="1" clearable controls-position="right" />
      </el-form-item>
      <el-form-item label="用户 ID" prop="userId">
        <el-input-number v-model="queryParams.userId" class="!w-180px" :min="1" clearable controls-position="right" />
      </el-form-item>
      <el-form-item label="模型 ID" prop="modelId">
        <el-input-number v-model="queryParams.modelId" class="!w-180px" :min="1" clearable controls-position="right" />
      </el-form-item>
      <el-form-item label="模型类型" prop="modelType">
        <el-select v-model="queryParams.modelType" class="!w-180px" clearable placeholder="请选择模型类型">
          <el-option v-for="item in AIGC_MODEL_TYPES" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
      <el-form-item label="渠道 ID" prop="providerId">
        <el-input-number v-model="queryParams.providerId" class="!w-180px" :min="1" clearable controls-position="right" />
      </el-form-item>
      <el-form-item label="能力" prop="capability">
        <el-select v-model="queryParams.capability" class="!w-220px" clearable placeholder="请选择能力">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" class="!w-160px" clearable placeholder="请选择状态">
          <el-option v-for="item in AIGC_USAGE_STATUSES" :key="item.value" :label="item.label" :value="item.value" />
        </el-select>
      </el-form-item>
  ...
  <ContentWrap>
    ...
    <el-row :gutter="16">
      ...
      <el-col :xs="24" :lg="12">
        <Echart :height="320" :options="capabilityBarOptions" />
      </el-col>
      <el-col :xs="24" :lg="12">
        <Echart :height="320" :options="modelTopOptions" />
      </el-col>
      <el-col :xs="24" :lg="12">
        <Echart :height="320" :options="failureRateOptions" />
      </el-col>
    </el-row>
    <el-table v-loading="statisticsLoading" :data="typeStatistics" :stripe="true" class="mt-16px">
      ...
      <el-table-column label="失败率" align="center" min-width="90">
        <template #default="scope">{{ formatRate(scope.row.failureRate) }}</template>
      </el-table-column>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
      ...
      <el-table-column label="能力" align="center" prop="capability" min-width="140">
        <template #default="scope">{{ getOptionLabel(AIGC_MODEL_CAPABILITIES, scope.row.capability) }}</template>
      </el-table-column>
      ...
      <el-table-column label="状态" align="center" prop="status" min-width="90">
        <template #default="scope">
          <el-tag :type="scope.row.status === 0 ? 'success' : 'danger'">
            {{ getOptionLabel(AIGC_USAGE_STATUSES, scope.row.status) }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="80" fixed="right">
        <template #default="scope"><el-button link type="primary" @click="openDetail(scope.row.id)">详情</el-button></template>
      </el-table-column>
    </el-table>
...
const statistics = ref<AigcModelUsageTypeStatisticsRespVO[]>([])
const total = ref(0)
const queryFormRef = ref()
const queryParams = reactive<AigcModelUsageLogPageReqVO>({
  pageNo: 1,
  pageSize: 10,
  taskId: undefined,
  userId: undefined,
...
const typeStatistics = computed(() => statistics.value.filter((item) => item.dimensionType === 'MODEL_TYPE'))
const capabilityStatistics = computed(() => statistics.value.filter((item) => item.dimensionType === 'CAPABILITY'))
const modelTopStatistics = computed(() => statistics.value.filter((item) => item.dimensionType === 'MODEL_TOP'))
const failureRateStatistics = computed(() => statistics.value.filter((item) => item.dimensionType === 'FAILURE_RATE'))
...
const getCapabilityLabel = (capability?: string) => {
  return getOptionLabel(AIGC_MODEL_CAPABILITIES, capability)
}
...
const capabilityBarOptions = computed<EChartsOption>(() => ({
  title: { text: '能力调用排行', left: 'center', textStyle: { fontSize: 14 } },
  tooltip: { trigger: 'axis' },
  grid: { left: 48, right: 20, top: 48, bottom: 64 },
  xAxis: {
    type: 'category',
    axisLabel: { interval: 0, rotate: 28 },
    data: capabilityStatistics.value.map((item) => getCapabilityLabel(item.capability))
  },
  yAxis: { type: 'value' },
  series: [{ name: '调用次数', type: 'bar', data: capabilityStatistics.value.map((item) => item.usageCount || 0) }]
}))

const modelTopOptions = computed<EChartsOption>(() => ({
  title: { text: '模型调用 TopN', left: 'center', textStyle: { fontSize: 14 } },
  tooltip: { trigger: 'axis' },
  ...
  xAxis: {
    ...
    data: modelTopStatistics.value.map((item) => item.modelName || `ID ${item.modelId}`)
  },
  yAxis: { type: 'value' },
  series: [{ name: '调用次数', type: 'bar', data: modelTopStatistics.value.map((item) => item.usageCount || 0) }]
}))

const failureRateOptions = computed<EChartsOption>(() => ({
  title: { text: '失败率排行', left: 'center', textStyle: { fontSize: 14 } },
  ...
  xAxis: {
    ...
    data: failureRateStatistics.value.map((item) => item.modelName || `ID ${item.modelId}`)
  },
  yAxis: { type: 'value', axisLabel: { formatter: '{value}%' } },
  series: [{ name: '失败率', type: 'bar', data: failureRateStatistics.value.map((item) => Number(item.failureRate || 0)) }]
}))
...
const getTypeStatistics = async () => {
  ...
  try {
    statistics.value = await AigcModelUsageApi.getUsageTypeStatistics(queryParams)
  } finally {
...
const handleQuery = () => {
  ...
  Promise.all([getList(), getTypeStatistics()])
}
