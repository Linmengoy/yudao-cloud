<template>
  ...
  <ContentWrap>
    ...
    >
      ...
      <el-table-column align="center" label="手机号" prop="mobile" width="130px">
        <template #default="scope">
            ...
            >
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  ...
                  <el-dropdown-item
                    v-if="checkPermi(['member:user:update-status'])"
                    command="handleUpdateStatus"
                  >
...
const handleUpdateStatus = async (row: UserApi.UserVO) => {
  const nextStatus =
    row.status === CommonStatusEnum.ENABLE ? CommonStatusEnum.DISABLE : CommonStatusEnum.ENABLE
  const actionText = nextStatus === CommonStatusEnum.ENABLE ? '启用' : '禁用'

  try {
    const result = await message.prompt(
      `请输入${actionText}会员「${row.nickname || row.mobile || row.id}」的原因`,
      '操作确认',
      {
    ...
    await message.confirm(`确认要${actionText}该会员吗？`)
