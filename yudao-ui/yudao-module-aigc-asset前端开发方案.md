...
- `/assets` 图片墙已改为安静静态展示：Muuri 布局不播放过渡动画，加载和刷新图标不做旋转动效，视频缩略图不在 hover 时自动播放。
...
- 项目页“修改项目显示图”弹窗左栏分页查询项目内图片资源，右栏分页查询当前用户所有图片资源；两栏都只选择 `assetId`，保存时更新项目 `coverAssetId`，不保存 URL。
...
- 资产墙渲染应保持安静：列表首次出现、分类切换、刷新、滚动加载和 Muuri 重排不使用可见动效；加载状态可以用静态图标或文字表达，不使用旋转动画。
- 图片和视频使用全尺寸可见预览，不裁剪、不套外层卡片、不显示圆角边和元数据信息；点击媒体进入资产详情。
- 视频资产在资产墙中默认显示封面或首帧，不因 hover 自动播放，避免列表滚动和分类切换时产生视觉干扰。
...
- 资产库分类数量来自 `/aigc/asset/my-category-counts`，切换全部、生成图片、上传图片、生成视频和其它文件时数量不随当前页加载情况漂移。
- 上传图片与生成图片在资产库中按 `sourceType` 分开展示；从 `/app` 加号资产选择器选中上传图片或生成图片时，只作为参考图输入，不改变后续生成结果资产类型。
...
- `/app` 支持直接粘贴剪贴板图片、文件上传和资产库选择作为参考图；参考图应立即显示，并缓存在本地，切换页面再回来仍能恢复。
- 多参考图请求完整保留数组字段，单参考图或旧模型兼容首张图字段；提交前必须展示可配置的模型参数弹窗并发送最终参数。
- `/app` 创建 canvas 项目后，参考图必须在 canvas 中形成实际图片节点，并连接到目标生成节点，不能只停留在快捷生成页状态。
- 项目页垃圾桶旁边的显示图按钮可打开两栏分页选择器：项目内图片资源和用户全部图片资源；保存只提交 `coverAssetId`。
- `/assets` 页面切换分类、刷新和滚动加载时应静态更新内容，不播放列表重排动效、不显示旋转加载动画、不触发视频 hover 自动播放。
- 私有 OSS/S3 URL 过期后，资产库、项目封面和 canvas 节点应通过资产详情或批量访问 URL 接口刷新，不依赖历史保存的签名 URL。
...
- 用户端新增交互动效只使用 `motion/react` 做局部短动效，不影响 React Flow 节点拖拽和画布 transform；资产库媒体墙属于高频浏览页面，默认不使用布局动效、旋转加载动效或 hover 自动播放。
