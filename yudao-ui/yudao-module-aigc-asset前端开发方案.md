...
- 已支持资产预览、下载、删除、标题/描述/标签编辑、可见性调整、审核状态展示和来源任务跳转。
...
- 搜索支持前端轻量过滤标题、资产编号和标签；若后端分页支持标题参数，则优先请求服务端过滤。
- 列表可采用安静的卡片网格或紧凑行列表，默认突出缩略图、标题、类型、来源项目或任务、审核状态、可见性、创建时间。
- 图片和视频分组展示时，图片使用自然比例缩略图，视频使用固定预览槽和封面占位，避免卡片高度跳动。
- 卡片展示封面、标题、类型、审核状态、可见性、文件大小、创建时间、下载次数，但不要把管理端字段堆满卡片。
- 列表项操作包括预览、详情、下载、删除、复制链接。
- 删除操作需要二次确认，成功后从列表移除或刷新当前页。
