...
当前实现状态：已完成主链路。`auth-store.tsx` 已移除 mock 主流程，登录 / 注册成功后保存 token、拉取用户资料和钱包，初始化时恢复登录态；已补齐 `loginByEmail`、`loginByEmailCode`、`resetPasswordByEmail`、按场景发送 / 校验邮箱验证码；refreshToken 失败后清理状态并通过 `auth-expired` 打开登录弹窗。当前已补齐 `redirectTo` 和 `authReason`，支持登录成功后回到原目标页面，并区分主动退出与登录过期。
...
fetchUser()
logout()
openLoginModal(mode, redirectTo, authReason)
closeLoginModal()
```
...
当前实现状态：已完成真实登录注册弹窗，支持手机号验证码、邮箱登录、邮箱验证码登录、邮箱注册、手机号密码登录、邮箱找回密码、验证码倒计时、表单校验、loading、错误提示和成功后进入 `/app`。邮箱注册已从一级 Tab 调整为邮箱登录表单内“立即注册”入口。已支持受保护页面触发登录时记录 `redirectTo`，登录成功后回到原始目标页面；主页“开始创作”已改为登录态判断，未登录时直接打开邮箱登录框并记录登录成功后进入 `/create/image`，已登录时直接进入创作页；生成提交上下文仍待结合 AIGC 提交流程继续补齐。
...
当前实现状态：已完成工作台布局级守卫。`src/app/(app)/layout.tsx` 未登录访问时跳回 `/` 并打开登录弹窗，同时记录原目标页面；登录成功后会跳回目标页面。主页 `src/app/(marketing)/page.tsx` 的“开始创作”不再直接跳受保护路由，而是未登录时直接打开登录框、已登录时进入 `/create/image`。主动退出登录会清理 token、user、wallet 和 React Query 缓存并返回首页，但不会自动弹出登录框；登录过期或访问受保护页面时才会打开登录弹窗。`RequireAuth.tsx` 已存在但当前未作为主要守卫挂载。任务、资产等非 React Query 私有缓存仍需随真实接口接入后补齐统一清理。
...
- 登录成功后顶部栏必须切换为已登录态，显示进入工作台、头像菜单、钱包 / 用量、个人中心、退出登录
- 已登录用户点击顶部主按钮不得再次打开登录弹窗
- 主动退出登录后返回首页，顶部栏恢复未登录态，但不自动弹出登录框
- 登录过期或未登录访问受保护页面时，才自动打开登录弹窗
- 首页“开始创作”作为核心创作入口，未登录时必须直接打开登录框，不先跳转到受保护路由；登录成功后进入 `/create/image`
...
- 首页“开始创作”未登录时直接打开登录框，登录成功后进入 `/create/image`
- 登录成功后回到原始目标页面已完成，继续生成提交原操作仍作为后续验收项
...
- 已完成工作台布局级登录守卫，未登录进入 `(app)` 路由会跳回首页并打开登录弹窗；首页“开始创作”已改为未登录直接打开登录框，登录成功后进入 `/create/image`
